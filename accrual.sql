CREATE OR REPLACE PROCEDURE EXT.SP_RPT_ACCRUAL_DETAIL(OUT FILENAME VARCHAR(100),
...
--st.VALUE * TO_DOUBLE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(sinc.genericattribute4 , ',', ''), '%', ''), 'USD', ''), 'quantity', ''), 'integer', '')) "Transaction Amount (USD)",
...
sinc.genericnumber6 "TI (LOC)",
--case when ifnull(sinc.genericnumber6,0)=0 then 0
--else ROUND(((co.ratevalue * sinc.genericnumber1)/ sinc.genericnumber6)) END
null "Tier rate",
--co.
null ratevalue,
--co.value
...
FROM tcmp.cs_salesorder so
INNER JOIN tcmp.cs_salestransaction st
    ON st.salesorderseq = so.salesorderseq
    and so.removedate=c_eot
    AND st.modelseq = 0
INNER JOIN  tcmp.cs_eventtype et
on et.removedate = c_eot
AND et.datatypeseq = st.eventtypeseq
INNER JOIN tcmp.cs_credit c
    ON c.salestransactionseq = st.salestransactionseq
    AND c.processingunitseq = st.processingunitseq
    and c.periodseq=V_periodseq
INNER JOIN EXT.RPT_ORGDATA h
ON h.positionseq=c.positionseq
and h.periodseq= V_periodseq
INNER JOIN cs_credittype ct
    ON ct.datatypeseq = c.credittypeseq
   AND ct.removedate = c_eot
LEFT JOIN tcmp.cs_pmcredittrace pmct
    ON pmct.creditseq = c.creditseq
    AND c.processingunitseq = pmct.processingunitseq
LEFT JOIN tcmp.cs_measurement mt
    ON pmct.measurementseq = mt.measurementseq
    AND mt.processingunitseq = pmct.processingunitseq
    AND mt.periodseq = pmct.targetperiodseq
    and mt.periodseq=c.periodseq
LEFT JOIN tcmp.cs_incentivepmtrace incentrace
    ON incentrace.measurementseq = mt.measurementseq
    and incentrace.targetperiodseq=mt.periodseq
    AND mt.processingunitseq = incentrace.processingunitseq
LEFT join cs_incentive sinc
 ON incentrace.incentiveseq = sinc.incentiveseq
 and incentrace.targetperiodseq=sinc.periodseq
 and sinc.positionseq=h.positionseq
 AND sinc.processingunitseq = incentrace.processingunitseq
 and sinc.periodseq=mt.periodseq
WHERE 1=1
AND h.processingunitseq = V_PUSEQ
AND h.periodseq = V_periodseq
and c.periodseq=V_periodseq
and sinc.periodseq = V_periodseq
AND IFNULL(sinc.genericboolean1, 0) = 1
AND TO_DOUBLE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(sinc.genericattribute2 , ',', ''), '%', ''), 'USD', ''), 'quantity', ''), 'integer', '')) > 0
AND SINC.genericattribute1 not in ('MBO','YOY Growth','CIP',
'Payment Adjustment')
;
...
st.value * c.genericnumber3*100 "Transaction Amount (USD)",
NULL "Quota",
...
END ,
CASE WHEN CREDIT_TYPE IN ('Direct Receiver','Revenue Adjustment','Roll up')
THEN NULL
ELSE TIER_RATE
END ,
CASE WHEN CREDIT_TYPE IN ('Direct Receiver','Revenue Adjustment','Roll up')
THEN NULL
ELSE ROUND(COMMISSION_LC, 4)
END ,
CASE WHEN CREDIT_TYPE IN ('Direct Receiver','Revenue Adjustment','Roll up')
THEN NULL
ELSE LOCAL_CURRENCY
